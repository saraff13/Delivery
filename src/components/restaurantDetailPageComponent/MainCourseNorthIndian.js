...
class MainCourseNorthIndian extends Component {
  state = {
    veg: false,
    dal: false,
    nonVeg: false,
  ...
  render() {
    ...
    const {veg, dal, nonVeg} = this.state;
    return (
      <>
        <TouchableOpacity
        ...
        <TouchableOpacity
          style={[
            styles.accordionBox,
            {borderBottomWidth: showOnlyVegDishes || dal ? 0 : 0.5},
            {borderTopWidth: veg ? 0.5 : 0},
          ]}
          onPress={() => this.setState({dal: !dal})}>
          <Text style={[styles.accordionText]}>
            {`Dal (${(dishesData && dishesData.total) || 0})`}
          </Text>
          <Icon
            name={dal ? 'chevron-up' : 'chevron-down'}
            style={[styles.accordionIcon]}
          />
        </TouchableOpacity>
        {dal && (
...
export default connect(mapStateToProps, {getdishes})(MainCourseNorthIndian);
